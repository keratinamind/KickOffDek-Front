...
function SidebarSetupVisual() {
    ...
    const handleChangeInput = (e) => {
        setIntput((currentState) => ({ ...currentState, [e.target.name]: e.target.files[0] }));
    };
    ...
    const clickSave = async () => {
        try {
            ...
            if (input.coverImage) {
                formData.append("project-image", input.coverImage);
                formData.append("project-image", input.campaignImage);
            } else {
                formData.append("project-image", project.coverImage);
                formData.append("project-image", input.campaignImage);
            }
    ...
    return (
        <div className="w-96 h-screen bg-gray-100 mx-5 my-5 mb-5">
            <div className="flex flex-row items-center">
                ...
                <h1 className="font-semibold text-md mr-3">Visuals</h1>
            ...
            <div className="flex flex-col py-2 my-2 w-full ">
                ...
                <div className="flex my-2 rounded items-center">
                    <div className="relative flex w-full flex-wrap items-stretch mb-3">
                        <label class="w-full flex flex-col items-center px-4 py-6 bg-white rounded-md shadow-md tracking-wide uppercase border border-blue cursor-pointer hover:bg-purple-600 hover:text-white text-purple-600 ease-linear transition-all duration-150">
                            ...
                            <input type="file" name="coverImage" onChange={handleChangeInput} />
                        </label>
                    </div>
                </div>
                <div className="flex flex-row items-center ">
                ...
                <div className="flex my-2 rounded items-center">
                    <div className="relative flex w-full flex-wrap items-stretch mb-3">
                        <label class="w-full flex flex-col items-center px-4 py-6 bg-white rounded-md shadow-md tracking-wide uppercase border border-blue cursor-pointer hover:bg-purple-600 hover:text-white text-purple-600 ease-linear transition-all duration-150">
                            <BsFillCloudArrowUpFill className="text-3xl mr-3" />
                            <span class="mt-2 text-base leading-normal">Select a file</span>
                            <input type="file" name="campaignImage" onChange={handleChangeInput} />
                        </label>
                    </div>
                </div>
                <div className="flex flex-row items-center ">
                ...
                <div className="flex flex-row items-center ">
                    <FiAlertCircle className="text-xs text-gray-400" />
                    <span className="text-xs text-gray-400 px-2">Use Youtube or Vimeo URL.</span>
                </div>
            </div>
        </div>
    );
